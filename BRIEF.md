...
**Session 14+15 — Fab Rate Card Adapters + JLCPCB** ✅ COMPLETE
`server/fabadapters/rate-card-engine.js`: shared pricing engine — linear area interpolation from reference price tables, with multipliers for layers (2/4/6/8L), surface finish (HASL-LF, ENIG, Immersion Silver, OSP), and copper weight (1oz/2oz). Supports all standard quantity breakpoints (5/10/25/50/100). Three thin adapter wrappers: `pcbtrain.js` (GBP, no assembly), `pcbway.js` (USD, assembly available), `eurocircuits.js` (EUR, assembly available) — each loads its rate card JSON and delegates to the shared engine. `server/fabadapters/jlcpcb.js`: full JLCPCB adapter with live API path (upload Gerbers → pre-review with dimension cross-check → online quote per quantity) + rate card fallback when API unavailable. `jlcpcb-signer.js`: HMAC-SHA256 request signing for JLCPCB API (canonical string = method + path + sorted params + timestamp). Singleton axios client with request signing interceptor and J-Trace-ID logging on every response. `data/fab_rates/jlcpcb.json` added as rate card fallback ($2 anchor for 100x100mm 2L). `server/fabquoter.js`: orchestrates all 4 fabs in parallel via `Promise.allSettled`, converts to GBP using FX rates from Redis (with hardcoded fallbacks: USD 0.79, EUR 0.855), applies manufacturing margin from `pricing.js` (26% new, 13% reorder, volume taper), writes `fab_quotes.json`. Wired into `worker.js` pipeline between packaging and files_ready (new `quoting` status). `/api/jobs/:id/quotes` endpoint updated to read `fab_quotes.json` and strip raw fab prices (only `customer_gbp` exposed).

**Session 16 — Manufacturing Order Flow**
Build `server/ordermanager.js`. Wire JLCPCB `createOrder()`. PCBTrain manual fulfilment (email notification). Test full order flow end-to-end in sandbox.
