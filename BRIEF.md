...
**Session 4 — User Accounts (Supabase)**
Build `server/accounts.js` using `@supabase/supabase-js`. Use the **shared Supabase project** (same DB as ops hub). Schema already defined in `eislapcb/operations` — tables: `users`, `orders`, `customers`. Implement customer register/login using Supabase Auth (not custom bcrypt — Supabase handles hashing and sessions). Add `designs` and `credits` tables via Supabase migration if not already in ops schema. Session tokens managed by Supabase (JWT, HttpOnly cookie). Test full CRUD. Note: NIST password rules (15-char min, HaveIBeenPwned check) are implemented in the ops hub's `src/lib/auth.ts` — replicate the same logic for consistency.
...
Build `server/notifier.js` using **Resend** (`resend` npm package, `RESEND_API_KEY`). From: `noreply@eisla.io`. Mirror the email patterns already implemented in `eislapcb/operations/src/lib/email.ts`. Test all notification types. Verify links resolve correctly.
